...
// Keep an unchanged graph byte-identical across builds. A wall-clock timestamp
// that advances on every invocation makes clean builds dirty and defeats
// reproducibility even when the catalogue and scoring inputs are unchanged.
let generated = new Date().toISOString();
if (fs.existsSync(outPath)) {
  try {
    const previous = JSON.parse(fs.readFileSync(outPath, "utf8"));
    const { generated: previousGenerated, ...previousContent } = previous;
    if (
      typeof previousGenerated === "string"
    ...
    console.warn(`[links] replacing unreadable prior graph: ${reason}`);
  }
}
const { _doc, ...graph } = content;
const out = { _doc, generated, ...graph };
